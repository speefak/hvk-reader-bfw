...
HKV_ID_LIST_FILE="${HKV_ID_LIST_FILE:-HKV_ID_list.lst}"
...
OPTIONS:
  ...
  -f, --id-file <path>     Use custom ID list file instead of default HKV_ID_list.lst
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -f|--id-file)
            shift
            [[ $# -eq 0 ]] && { echo "Error: -f requires path"; exit 1; }
            HKV_ID_LIST_FILE="$1"
            shift
            ;;             
        -h|--help) show_help ;;
